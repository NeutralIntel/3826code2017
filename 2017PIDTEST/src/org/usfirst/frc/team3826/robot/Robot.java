...
public class Robot extends IterativeRobot {
	...
	public void autonomousPeriodic() {
		...
		if (centerX <240){//From left border to Center-80 pixels
			...
			if(centerX > 240 && centerX < 400 && autoCounter == 4 && Math.abs(rightSide.getRate()) < 50){//And so that we are in step 5
				targetRot = 0;
			}
	...
	angleDisplace = Math.abs(angleNow - autoSetpoint);
        ...
        if(angleNow > autoSetpoint && angleDisplace > 15){//We are over 15 deg off (to the right)
        	rotSpeed = 0.018525 * (angleDisplace+6);
        }
        	...
        	rotSpeed = -0.015925 * (angleDisplace+6);
        }
        ...
        switch(mode) {//Case switch, input number based on value from smart dashboard
        	...
        	case 4://Middle Gear
            	...
            	if(autoCounter == 0){//Step 1 - Forward
            		...
            		else if(rightSide.get() < 750 && rightSide.getRate() > 10){//rightSide.get() < 1400){
        				ourRobot.arcadeDrive(-.6,.25*angleNow);//Move forward until encoder value is passed
        			}
            		else if(rightSide.getRate() < 10 && rightSide.get() > 50 && rightSide.get() < 750){
        				ourRobot.arcadeDrive(-.7,.25*angleNow);
        			}
            		else if(rightSide.get() < 1950 && rightSide.getRate() > 50){
        				ourRobot.arcadeDrive(-0.45,.25*angleNow);
        				autoTime = 0;
        			}
        			else if(rightSide.getRate() < 50 && rightSide.get() < 1725 && rightSide.get() > 750){
        				ourRobot.arcadeDrive(-0.675,.25*angleNow);
        			}
        			else if(rightSide.getRate() < 50 && rightSide.get() > 1725 && rightSide.get() < 1900){
        			...
        			if(rightSide.get() > 1950 && autoTimer.get() > 8){
        				ourRobot.arcadeDrive(0,0);//stop
        				autoCounter = 1;//next step
        			}
        		}
        	...
        	case 9://Right Shooting
        		...
        		if(autoCounter == 0 && mode == 9){
        			...
        			else if(rightSide.get() < 2400 && (autoInt == 1 || autoInt == 2)){
        				ourRobot.arcadeDrive(-.65,0);
        				autoInt = 2;
        			}
        	...
        	case 10://Left Shooting
        		...
        		if(autoCounter == 0 && mode == 10){
        			...
        			else if(rightSide.get() < 1600 && (autoInt == 1 || autoInt == 2)){
        				ourRobot.arcadeDrive(-.65,0);
        				autoInt = 2;
        			}
		...
		if(autoCounter == 0 && runAuto){//Step 1 - Move Forward
			if(rightSide.get() < 25 && leftSide.get() < 25 && autoTimer.get() > 2){
				autoCounter = 20;
			}
			if(rightSide.get() < 700 *1.425 && leftSide.get()<2500){
        		ourRobot.arcadeDrive(-.6,.325*angleNow);
        	}
			else if(rightSide.get() < 700 *1.425 && leftSide.get()>=2500){
				autoCounter=20;
			}
			else if(rightSide.get() < 850 *1.425){
        		ourRobot.arcadeDrive(-.55,.325*angleNow);
        	}
			else if(rightSide.get() < 1500 *1.425){
        		ourRobot.arcadeDrive(-.45,.325*angleNow);
        	}
			else if(rightSide.get() < (1900 *1.425) +leftChange){//
        		ourRobot.arcadeDrive(-.4,.325*angleNow);
        	}
			else if( leftSide.get() > 3500){
				autoCounter=20;
			}
        	else{
        		ourRobot.arcadeDrive(0,0);//stop
        	...
        	ourRobot.arcadeDrive(0,0);//stop
      		if(rightSide.getRate() < 10){
        		...
        		if(rotSpeed == 0){
        			if(angleNow<=autoSetpoint+5 && angleNow>=autoSetpoint-5){
        				if(Math.abs(rightSide.getRate())<10){
        				ourRobot.arcadeDrive(0,0);//stop
            			Timer.delay(.0275);
            			rightSide.reset();//Reset encoders
            			rightSide.reset();
            			turnAdj=0;
        			...
        			if(rotSpeed >= 0.65){//Limits speed to 80%
                		ourRobot.arcadeDrive(0,.65);
                	}
        			else if(rotSpeed <= -0.7){//Limits speed to  80%
                		ourRobot.arcadeDrive(0,-.65);
                	}
                	else{
                		if(rotSpeed>0){
                		ourRobot.arcadeDrive(0,rotSpeed);//+((.25*Math.abs(rotSpeed*rotSpeed*rotSpeed)*turnAdj)));//turn at speed based on angle (See up above for how rotSpeed is calculated)
                		}
                		else if(rotSpeed<0){
                			ourRobot.arcadeDrive(0,rotSpeed);//-((.25*Math.abs(rotSpeed*rotSpeed*rotSpeed)*turnAdj)));
                		}/*
                		if(Math.abs(rightSide.getRate())<25 && angleDisplace<20 && !timing){
                			if(angleDisplace < 5){
                				ourRobot.arcadeDrive(0,0);//stop
                    			Timer.delay(.025);
                    			rightSide.reset();//Reset encoders
                			...
                			stallTime=autoTimer.get();
                			timing=true;
                		}
                		else if(Math.abs(rightSide.getRate())<25&& angleDisplace<20 && timing){
                			if(angleDisplace < 5){
                				ourRobot.arcadeDrive(0,0);//stop
                    			Timer.delay(.025);
                    			rightSide.reset();//Reset encoders
                			...
                			if(stallTime +0.1 <= autoTimer.get() && turnAdj < 4){
                				turnAdj++;
                				timing=false;
                			}
                		}*/
        	...
        	if (rightSide.get() <750 *1.425){//rightSide.get()<750){
        		ourRobot.arcadeDrive(-.575,0.25*(angleNow-autoSetpoint));//Move forward until encoders past setpoint
        	}
        	else{
        		ourRobot.arcadeDrive(0,0);//stop
        		if(rightSide.getRate()<10){
        	...
        	if(centerX > 240 && centerX < 400){//If target is already centered
        		...
        		if(targetRot > .25){//If center is on the left, turn based on distance from center
           			...
           			Timer.delay(.25);//wait
           			if(rightSide.getRate()<10){//rightSide.getRate()<10){//make sure bot is stopped
           				ourRobot.arcadeDrive(0,0);//stop
           				Timer.delay(.25);
           				autoCounter = 5;//next step
           			}
        		...
        		else if(Math.abs(rightSide.getRate())<10){
        			if(turnAdj<8){turnAdj++;}
        			if(targetRot > .25){//If center is on the left, turn based on distance from center
               			if(targetRot < .345){
               				ourRobot.arcadeDrive(0,.35+(.025*turnAdj));
               			}
               			else if(targetRot > .5){
               				ourRobot.arcadeDrive(0,.48+(.025*turnAdj));
               			}
               			else{
               				ourRobot.arcadeDrive(0,targetRot+(.025*turnAdj));
               			}
               		}
               		else if(targetRot < -.25){//If center is on the right, turn based on distance from center
               			if(targetRot > -.345-(.025*turnAdj)){
               				ourRobot.arcadeDrive(0,-.35);
               			}
               			else if(targetRot < -.475){
               				ourRobot.arcadeDrive(0,-.48-(.025*turnAdj));
               				...
               				ourRobot.arcadeDrive(0,targetRot-(.025*turnAdj));//See up above for how targetRot is calculated
               			}
               		}
               		else{//If target is now centered,
               			Timer.delay(.25);//wait
        	...
        	if(rightSide.get() < 250 *1.425){
        		ourRobot.arcadeDrive(-.6,0);//move until past encoders
        		}
        	else{
        		ourRobot.arcadeDrive(0,0);//stop
        		if(!safeMode){
        			if(flagOne.getValue() > 3000 && rightSide.getRate() < 10){//if door closed
				...
				else if(safeMode && rightSide.getRate() < 10){
					autoCounter = 20;
				}	
			}
        }
    ...
    public void teleopPeriodic() {
    	...
    	if(xBox.getRawAxis(2)>.1){//Left Trigger
    		shooter.changeControlMode(TalonControlMode.Speed);
    		shooter.set(-18500);//Setpoint for PID 18050
    	}
    	else{
    		shooter.changeControlMode(TalonControlMode.Voltage);//Change mode to voltage in order to stop the shooter
    		shooter.set(0);
    		...
    		if(shooter.getEncVelocity() < -18275){//Speed we shoot at 17975
    			atSpeed = true;
    		}
    		else{
    			atSpeed = false;
